...
for i in range(changing_dim_energy.shape[1]):
    if np.std(changing_dim_energy[:,i]) != 0:
        changing_dim_energy[:,i] = (changing_dim_energy[:,i]- np.average(changing_dim_energy[:,i]))/np.std(changing_dim_energy[:,i])

#dimension reduction
after_dim_red= np.empty((height*width,3))
from sklearn.decomposition import PCA
pca = PCA(n_components=10)
after_dim_red = pca.fit_transform(changing_dim_energy)
...
cv2.imwrite('segmentation_PCA.jpg', Out_image)
kernel = np.ones((8,8), np.float32)
close = cv2.morphologyEx(Out_image, cv2.MORPH_CLOSE, kernel)
close = cv2.morphologyEx(close, cv2.MORPH_CLOSE, kernel)
close = cv2.morphologyEx(close, cv2.MORPH_CLOSE, kernel)
im= cv2.imwrite('segmentation_PCA_closing.jpg', close)
